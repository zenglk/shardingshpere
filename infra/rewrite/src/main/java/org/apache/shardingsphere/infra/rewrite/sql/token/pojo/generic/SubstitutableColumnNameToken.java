    ...
    private String getColumnName(final ColumnProjection columnProjection, final Map<String, String> logicActualTableNames) {
        ...
        if (!Strings.isNullOrEmpty(owner)) {
            ...
            if (columnProjection.getAlias().isPresent()) {
                builder.append(quoteCharacter.wrap(columnProjection.getAlias().get()).toUpperCase());
                return builder.toString();
            }
        }
        builder.append(quoteCharacter.wrap(columnProjection.getName()).toUpperCase());
        if (columnProjection.getAlias().isPresent()) {
            builder.append(" AS ").append(quoteCharacter.wrap(columnProjection.getAlias().get()).toUpperCase());
        }
        return builder.toString();
    }
}
